...
public class Mapper extends GUI {
	...
	public void findpath() {
		if(Startnode!=null&&Endnode!=null&&Startnode!=Endnode){ // when they both not equals null will show some information.
			graph.setStartEndnode(Startnode,Endnode);
			List<Segment> seg = Pathfinding.Asearch(Startnode, Endnode);
			graph.setHighlightSegment(seg);
	...
	public String totalcost(List<Segment> seg){
		...
		for(Segment s : seg){
			System.out.println(roads.add(s.road));
	...
	protected void onMove(Move m) {
		...
		if(m==GUI.Move.ArticulationPoints){
			...
			graph.ArticulationPoints = Pathfinding.ArticulationPoints(graph.nodes);
			getTextOutputArea().setText("Total Articulation Points: "  +  graph.ArticulationPoints.size());
